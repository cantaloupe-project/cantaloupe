 ...
 *
 * <p>HTTP/1.1, HTTPS/1.1, and HTTPS/2.0 are supported.</p>
 *
 ...
 */
class HttpSource2 extends AbstractSource implements StreamSource {
    ...
    static final Logger LOGGER = LoggerFactory.getLogger(HttpSource2.class);
    ...
    private Format inferSourceFormatFromHEADResponse() {
        ...
        try {
            ...
            final HttpSource2.HEADResponseInfo responseInfo = fetchHEADResponseInfo();

            if (responseInfo.status >= 200 && responseInfo.status < 300) {
                String field = responseInfo.headers.get("Content-Type");
                if (field != null) {
                    format = MediaType.fromContentType(field).toFormat();
                    if (Format.UNKNOWN.equals(format)) {
    ...
    private Format inferSourceFormatFromMagicBytes() {
        ...
        try {
            ...
            if (fetchHEADResponseInfo().acceptsRanges()) {
                final HttpSource2.RangedGETResponseInfo responseInfo
