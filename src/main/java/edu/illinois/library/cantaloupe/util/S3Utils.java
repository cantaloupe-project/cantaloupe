...
public final class S3Utils {
     ...
     */
    public static void createBucket(S3AsyncClient client, String bucketName) {
        try {
            emptyBucket(client, bucketName);
        } catch (CompletionException e) {
            if (e.getCause() instanceof NoSuchBucketException) {
                client.createBucket(CreateBucketRequest.builder()
                        .bucket(bucketName).build()).join();
            } else {
                throw e;
            }
        }
     ...
     */
    public static void deleteBucket(S3AsyncClient client, String bucketName) {
        emptyBucket(client, bucketName);
     ...
     */
    public static int emptyBucket(S3AsyncClient client, String bucketName) {
