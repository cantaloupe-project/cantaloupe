...
/**
 * Spring Boot test for IIIF v1 Identifier Controller.
...
class IdentifierControllerTest {
    ...
    void testRedirectToInfo_BasicIdentifier() throws Exception {
        ...
        mockMvc.perform(get("/iiif/1/{identifier}", identifier))
                .andExpect(status().isSeeOther())
                .andExpect(header().string("Location", containsString("/iiif/1/" + identifier + "/info.json")));
    }
    ...
    void testRedirectToInfo_SpecialCharacters() throws Exception {
        ...
        mockMvc.perform(get("/iiif/1/{identifier}", identifier))
                .andExpect(status().isSeeOther())
                .andExpect(header().string("Location", is("/iiif/1/" + identifier + "/info.json")));
    }
    ...
    void testRedirectToInfo_WithContextPath() throws Exception {
        ...
        mockMvc.perform(get("/cantaloupe/iiif/1/{identifier}", identifier)
                .contextPath("/cantaloupe"))
                .andExpect(status().isSeeOther())
                .andExpect(header().string("Location", is("/cantaloupe/iiif/1/" + identifier + "/info.json")));
    }
    ...
    void testRedirectToInfo_EndpointDisabled() throws Exception {
        ...
        mockMvc.perform(get("/iiif/1/test-image"))
    ...
    void testOptionsIdentifier() throws Exception {
        mockMvc.perform(options("/iiif/1/test-image"))
    ...
    void testRedirectToInfo_LongIdentifier() throws Exception {
        ...
        mockMvc.perform(get("/iiif/1/{identifier}", longIdentifier))
                .andExpect(status().isSeeOther())
                .andExpect(header().string("Location", containsString("/iiif/1/" + longIdentifier + "/info.json")));
    }
